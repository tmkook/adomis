...
export default class AdminAppCommand extends BaseCommand {
    ...
    async run() {
        if (!fs.existsSync(this.app.viewsPath('admin'))) {
            fs.cpSync(kmolaRoot + '/stubs/resources/views/admin', this.app.viewsPath('admin'), { recursive: true });
        ...
        if (!fs.existsSync(this.app.languageFilesPath('en/admin.json'))) {
            fs.cpSync(kmolaRoot + '/stubs/resources/lang', this.app.languageFilesPath(), { recursive: true });
